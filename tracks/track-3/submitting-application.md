...
- Keep the data in **`./corpus`** on the VM, which compose mounts read-only at the same path inside the container, with the layout given in [Building your Application]({% link tracks/track-3/building-application.md %}#running-in-a-container).
