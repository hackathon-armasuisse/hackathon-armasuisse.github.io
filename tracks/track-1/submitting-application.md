...
- Keep the data mounted read-only at **`/corpus`** (compose mounts `./data` there, or set `CORPUS_DIR` on the host), with the layout given in [Building your Application]({% link tracks/track-1/building-application.md %}#running-in-a-container).
